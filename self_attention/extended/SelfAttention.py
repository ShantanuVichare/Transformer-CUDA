...
class SelfAttentionFunction(torch.autograd.Function):
  ...
  def backward(ctx, grad_out):
    outputs = self_attention.backward(grad_out.contiguous(), *ctx.saved_tensors, ctx.vkq, ctx.weights, ctx.consts)
    outputs += [None, None]
...
class SelfAttention(nn.Module):
  ...
  def forward(self, values, keys, query, mask):
    ...
    return SelfAttentionFunction.apply(
      values, keys, query,
      self.W_values, self.W_keys, self.W_query, self.W_out, self.B_out,
      mask, [N, seq_len, self.heads, self.head_dim, self.embed_size]
      )
