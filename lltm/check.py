...
def check_forward(variables, verbose):
...
def check_backward(variables, verbose):
    baseline_values = python.lltm_baseline.LLTMFunction.apply(*variables)
...
if "forward" in options.direction:
    check_forward(variables, options.verbose)

if "backward" in options.direction:
    check_backward(variables, options.verbose)
